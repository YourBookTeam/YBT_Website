...
function Pagination({ totalPages, currentPage, handlePrevious, handleNext, handlePageClick }) {
  return (
    <div className="flex gap-4 items-center justify-center md:px-30 lg:px-50 xl:px-150 py-20">
        <button onClick={handlePrevious} className={`w-10 h-10 rounded-full flex items-center justify-center ${currentPage == 1 ? "bg-light-gray" : "cursor-pointer bg-black"}`}>
        ...
        <button onClick={handleNext} className={`w-10 h-10 rounded-full flex items-center justify-center ${currentPage == totalPages ? "bg-light-gray" : "cursor-pointer bg-black"}`}>
