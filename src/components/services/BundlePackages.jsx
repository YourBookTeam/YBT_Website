...
function BundlePackages(){
    ...
    const [currentPackage, setCurrentPackage] = useState(bundleData.completePackage);

    const handleToggle = ()=> {
        const currentToggle = !toggleSwitch;
        setToggleSwitch(currentToggle);
        setCurrentPackage(currentToggle ? bundleData.completePackage : bundleData.authorPackage);
    }
    ...
    return(
        <div className="flex flex-col justify-center items-center gap-8">
            <div className="w-[95%] h-30 bg-white rounded-lg shadow-sm flex flex-col items-center gap-2 py-2 text-[13px] font-bold">
                <button 
                    className={`flex justify-center items-center rounded-lg cursor-pointer py-2 px-4 w-[95%] h-12 transition-colors duration-300 ease-in-out ${toggleSwitch ? " bg-gold text-white" : " bg-white text-black"}`}
